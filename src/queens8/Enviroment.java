...
public class Enviroment {
	...
	public void printSuccessorStates() {
		...
		if (numQueens < 8) {
			...
			for (int i = 0; i < 8; i++) {
				...
				for (int j = 0; j < 8; j++) {
					// if we find a queen generate 7 successor states of the
					// environment by moving the queen on that column
					if (enviromentState[i][j] == 1) {
						successorEnvState[i][j] = 0;
						for (int n = 1; n < 8; n++) {
							// set to 1 then 0 so as to restart the state for
							// the successor
	...
	public static Enviroment generateRandomInitialEnv() {
		...
		while (Queens8Driver.hueristicCost(initEnv) != 0) {
			...
			y = rand.nextInt((7 - 0) + 1);
			for (int i = 0; i < numQueens; i++) {
				while (initialstate[y][i] == 1) {// we found a queen there
													// already
					y = rand.nextInt((7 - 0) + 1);
